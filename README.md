...
**팀장** — 프로젝트 전반을 총괄하며 기획·디자인·개발·배포까지 전 과정을 주도했습니다.

- **기획·컨셉 총괄**: 예비 출판 마케터의 관점에서 "책 문장을 광고가 아니라 날씨처럼 매일 마주치게 한다"는 핵심 컨셉을 설계하고, 앱의 방향과 사용자 경험 흐름을 정의했습니다.
- **UI/UX 디자인 & 손그림 아트워크**: 날씨 배경, 날씨 아이콘, 앱 아이콘, 크레용 드로잉 도구 등 앱 전반의 손그림 에셋을 직접 제작하고 화면 레이아웃을 설계했습니다.
- **iOS 개발 주도**: Claude Code와의 페어 프로그래밍으로 요구사항을 자연어로 전달하고, SwiftUI·WidgetKit·PencilKit 기반 기능을 함께 구현했습니다.
- **콘텐츠 큐레이션**: Google Sheets 기반으로 매일의 책 문장을 직접 선별·관리하는 큐레이션 체계를 운영했습니다.
- **품질 검증(QA)**: 시뮬레이터와 실기기에서 반복적으로 테스트하며 위젯·드로잉·날씨 연동 등의 문제를 발견하고 개선했습니다.
- **배포 파이프라인 구축**: 유료 개발자 계정 없이도 실기기에 지속 배포할 수 있도록 AltStore/AltServer 기반 무료 배포 구조를 직접 설계·운영했습니다.
