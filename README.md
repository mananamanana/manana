...
이용자는 매일 아침 오늘의 날씨 배경 위에 얹힌 책 문장을 만나고, 그 위에 손그림을 더해 하루를 기록합니다. 문장이 홍보처럼 소비되지 않고, 하루의 감정과 함께 자연스럽게 마음에 남도록 설계했습니다. 기록은 달력과 보관함에 쌓이고, 홈 화면·잠금화면 위젯을 통해 앱을 열지 않고도 매일 새로운 문장을 마주치게 됩니다.
...
- **팀 프로젝트 연동**: 별도로 기여한 기능(펜 아이콘 → 크레용 아이콘 교체, 위젯 자정 갱신 로직 등)을 GitHub을 통해 pull 받아 통합
...
### ☁️ 추원지 (팀장)
- **기획·컨셉 총괄**: 예비 출판 마케터의 관점에서 "책 문장을 광고가 아니라 날씨처럼 매일 마주치게 한다"는 핵심 컨셉과 앱의 방향을 설계했습니다.
- **UI/UX 디자인 & 손그림 아트워크**: 날씨 배경, 날씨 아이콘, 앱 아이콘, 크레용 드로잉 도구 등 앱 전반의 손그림 에셋을 직접 제작하고 화면 레이아웃을 설계했습니다.
- **협업 관리 & 문서화**: 팀원의 기여를 GitHub으로 통합하고, README·설치 가이드·포트폴리오 자료를 작성했습니다.

### ☁️ 이찬영
- **기획·컨셉**: 앱의 방향과 사용자 경험 흐름 정의에 참여했습니다.
- **iOS 개발**: Claude Code와의 페어 프로그래밍으로 SwiftUI·WidgetKit·PencilKit 기반 기능을 구현했습니다.
- **콘텐츠 큐레이션**: Google Sheets 기반으로 매일의 책 문장을 선별·관리하는 큐레이션 체계를 운영했습니다.
- **품질 검증(QA)**: 시뮬레이터와 실기기에서 반복적으로 테스트하며 위젯·드로잉 등의 문제를 발견하고 개선했습니다.
- **배포 파이프라인 구축**: 유료 개발자 계정 없이도 실기기에 지속 배포할 수 있도록 AltStore/AltServer 기반 무료 배포 구조를 설계·운영했습니다.

### ☁️ 이성재
- **기획·컨셉**: 앱의 방향과 사용자 경험 흐름 정의에 참여했습니다.
- **iOS 개발**: Claude Code와의 페어 프로그래밍으로 SwiftUI 기반 기능을 구현했습니다.
- **날씨 연동**: 기상청 단기예보 API와 Cloudflare Worker 프록시를 통한 날씨 데이터 연동을 담당했습니다.
- **문서화**: README·설치 가이드 등 문서 작성에 참여했습니다.
